    ...
    try {
      ...
      if (data.user) {
        ...
        if (data.session) {
          ...
          console.log("Session available, creating setup session...");
  ...
  return (
    <main className="min-h-screen bg-white flex flex-col items-center justify-center">
      <div className="w-full max-w-sm space-y-6">
            ...
            <h1 className="text-4xl font-light tracking-wide text-gray-900" style={{ fontFamily: 'Helvetica, Arial, sans-serif' }}>Sign Up</h1>
          ...
          <button
            ...
            onClick={async () => {
              ...
              const { error } = await supabase.auth.signInWithOAuth({
                provider: 'google',
                options: {
                  redirectTo: `${window.location.origin}/signup-redirect`
                }
              });
              if (error) {
                setError(error.message);
